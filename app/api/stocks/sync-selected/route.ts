...
/**
 * Выборочная синхронизация остатков.
 *
 * Полная синхронизация (app/api/stocks/sync-all) идёт заданием в несколько
 * шагов: тысячи товаров не помещаются в один запрос. Здесь выбор ограничен
 * полусотней позиций, поэтому всё делается за один заход — так проще
 * проверить расчёт на нескольких артикулах, не трогая остальной ассортимент.
 *
 * Формула, гард от некорректных значений и запись в журнал — те же самые:
 * используются lib/stock-math и lib/stock-log, отдельной арифметики здесь нет.
 */
...
export async function POST(request: Request) {
  ...
  await startSyncRun(db, { runId, trigger: "manual", actorEmail: auth.user.email, osvUploadId });
  ...
  if (logRows.length > 0) await logStockRows(db, runId, auth.user.email, logRows);
  ...
  if (wbWarehouses.length > 0) {
    ...
    ).bind(wbSent > 0 ? "success" : "error", wbSent, `Выборочная синхронизация: позиций ${sourceSkus.length}, отправлено ${wbSent}.`));
  }
  if (ozonWarehouses.length > 0) {
    statements.push(db.prepare(
      `INSERT INTO sync_events (marketplace_id, direction, kind, status, item_count, message)
       VALUES ('ozon', 'outbound', 'stocks', ?, ?, ?)`,
    ).bind(ozonSent > 0 ? "success" : "error", ozonSent, `Выборочная синхронизация: позиций ${sourceSkus.length}, отправлено ${ozonSent} пар товар–склад.`));
  }
