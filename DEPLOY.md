...
Вместе с кодом обновляются описания служб и расписания: `update.sh` сравнивает
файлы `deploy/*.service` и `deploy/*.timer` с установленными в
`/etc/systemd/system` и переустанавливает только изменившиеся, после чего делает
`daemon-reload`. Поэтому смена расписания — это обычная правка файла таймера и
push в `main`, заходить в консоль сервера не нужно. Исключение — `nginx-site.conf`:
в него при установке подставляются домен и порт, поэтому он обновляется только
через `install.sh`.
