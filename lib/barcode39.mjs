...
/**
 * Номер задания из скана листа подбора.
 *
 * Сканер «печатает» штрихкод как клавиатура: при русской раскладке T
 * превращается в «Е», а при латинской может прийти строчная t. Всё это —
 * один и тот же лист. УИН (16 цифр) сюда не подходит: у него нет буквы.
 *
