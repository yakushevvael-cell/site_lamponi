...
/**
 * Единое написание числового размера: «17», «17,0», «17.0» и «17,00» — это
 * один размер кольца, и в 1С он встречается во всех этих видах, иногда у
 * одного и того же артикула. Раньше каждое написание становилось отдельной
 * позицией: остаток делился между ними, а площадки сопоставлялись с одной
 * из двух. Теперь числовой размер хранится как число с запятой («17», «16,5»),
 * нечисловые (XL, «16,0 +», Б/Р) не трогаем.
 *
 * @param {string} size
 */
export function canonicalOsvSize(size) {
  const compact = String(size).replace(/\s+/g, "").replace(",", ".");
  if (!/^\d+(?:\.\d+)?$/.test(compact)) return size;
  const numeric = Number(compact);
  return Number.isFinite(numeric) ? String(numeric).replace(".", ",") : size;
}

export function variantKey(article, size) {
...
export function parseOsvSheet(sources) {
  ...
  for (const row of dataRows) {
    ...
    const size = value === "<Пустое субконто4>" ? null : canonicalOsvSize(value);
