...
export async function pushStocksForSkus(options: PushStocksOptions): Promise<PushStocksResult | { error: string }> {
  ...
  if (wbWarehouses.length === 0 && ozonWarehouses.length === 0) {
    ...
    return {
      error: "Нет складов с включённой выгрузкой остатков. Включите нужный склад на вкладке «Склады площадок» — "
        + "в ручном режиме отметьте там «Только включить, ничего не отправлять», и остатки на площадке не изменятся.",
    };
  }
