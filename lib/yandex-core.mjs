 ...
 *
 * @param {Array<{ id: number, count?: number, uin?: string | null, uins?: string[] }>} items
 */
export function buildYandexBoxes(items) {
  ...
  return [{
    items: rows.map((item) => {
      ...
      // Одна штука — один УИН: строке «4 шт.» передаются четыре.
      const uins = (Array.isArray(item.uins) ? item.uins : [item.uin])
        .map((value) => String(value ?? "").trim())
        .filter(Boolean);
      if (uins.length > 0) box.instances = uins.map((uin) => ({ uin }));
      return box;
    }),
  }];
}
