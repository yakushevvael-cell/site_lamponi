...
export function buildDeliveryRequest(input) {
  ...
  const items = (input.items ?? []).map((item) => ({
    ...
    billing_details: {
      ...
      // Заказ уже оплачен на Маркете, курьер чек не пробивает, поэтому по
      // умолчанию ставка не указывается. Значение задаётся в настройках.
      nds: normalizeNds(input.nds),
    },
